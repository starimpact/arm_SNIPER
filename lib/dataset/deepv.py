...
class deepv(IMDB):
    ...
    def evaluate_detections(self, detections):
        ...
        if not os.path.exists(result_dir):
            os.mkdir(result_dir)
        year_folder = os.path.join(self.result_path, 'results', 'deepv')
    ...
    def write_pascal_segmentation_result(self, pred_segmentations):
        ...
        if not os.path.exists(year_folder):
            os.mkdir(year_folder)
        res_file_folder = os.path.join(self.result_path, 'results', 'deepv', 'Segmentation')
        if not os.path.exists(res_file_folder):
            os.mkdir(res_file_folder)

        result_dir = os.path.join(self.result_path, 'results', 'deepv', 'Segmentation')
        if not os.path.exists(result_dir):
            os.mkdir(result_dir)
