...
from inference import imdb_detection_wrapper, imdb_rpn_detection_wrapper
...
def parser():
    ...
    arg_parser.add_argument('--cache_prefix', dest='cache_prefix', help='Prefix used for snapshotting the network',
                            default='SNIPER', type=str)
...
#    args.vis = True
    ...
    if config.TEST.EXTRACT_PROPOSALS:
        imdb_proposal_extraction_wrapper(sym_inst, config, imdb, roidb, context, arg_params, aux_params, args.vis, do_nms=True)
    else:
        imdb_rpn_detection_wrapper(sym_inst, config, imdb, roidb, context, arg_params, aux_params, args.vis, args.cache_prefix)
